...
fn cors_response(endpoint: &str, credentialed: bool) -> FetchResponse {
    let mut response = FetchResponse::synthetic(url(endpoint), 200, Some("text/plain"), b"ok".to_vec());
    response.headers.append_raw(
        "access-control-allow-origin",
        if credentialed { "http://page.test" } else { "*" },
    );
    if credentialed {
        response.headers.append_raw("access-control-allow-credentials", "true");
    }
    response
}
...
fn cross_origin_response_exposes_only_cors_safelisted_headers_by_default() {
    ...
    response.headers.append_raw("expires", "Wed, 21 Oct 2026 07:28:00 GMT");
    response.headers.append_raw("last-modified", "Tue, 20 Oct 2026 07:28:00 GMT");
    ...
    let (mut browser, transport) = browser_for("http://page.test/index.html", script, endpoint, response);
    complete_fetch(&mut browser, &transport);
    assert_eq!(browser.document().runtime.console, vec!["cache", "language", "length", "type", "expires", "modified", "pragma", "hidden", "no-acao"]);
}
...
fn expose_headers_adds_named_headers_case_insensitively() {
    let endpoint = "http://api.test/data";
    let mut response = cors_response(endpoint, false);
    response.headers.append_raw("access-control-expose-headers", "X-Request-ID, x-trace");
    ...
    let (mut browser, transport) = browser_for("http://page.test/index.html", script, endpoint, response);
    complete_fetch(&mut browser, &transport);
    assert_eq!(browser.document().runtime.console, vec!["req-1", "trace-1", "hidden", "no-aceh"]);
}
...
fn noncredentialed_expose_headers_wildcard_exposes_all_non_forbidden_headers() {
    let endpoint = "http://api.test/data";
    let mut response = cors_response(endpoint, false);
    response.headers.append_raw("access-control-expose-headers", "*");
    ...
    let (mut browser, transport) = browser_for("http://page.test/index.html", script, endpoint, response);
    complete_fetch(&mut browser, &transport);
    assert_eq!(browser.document().runtime.console, vec!["visible", "acao", "no-cookie"]);
}
...
fn credentialed_expose_headers_wildcard_is_literal_not_a_wildcard() {
    ...
    let (mut browser, transport) = browser_for("http://page.test/index.html", script, endpoint, response);
    complete_fetch(&mut browser, &transport);
    assert_eq!(browser.document().runtime.console, vec!["hidden", "literal-star"]);
}
...
fn credentialed_response_can_explicitly_expose_a_custom_header() {
    let endpoint = "http://api.test/data";
    let mut response = cors_response(endpoint, true);
    response.headers.append_raw("access-control-expose-headers", "X-Secret");
    response.headers.append_raw("x-secret", "visible");
    let script = r#"fetch("http://api.test/data", { credentials: "include" }).then(function (r) {
        console.log(r.headers.get("x-secret"));
    });"#;
